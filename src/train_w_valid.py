...
@torch.no_grad()
def evaluate_validation_loss(model, val_loader, transport, rae, device, autocast_kwargs, world_size):
    """
...
@torch.no_grad()
def eval_validation_loss(model, val_loader, transport, rae, device, autocast_kwargs, world_size):
    """
...
def main(args):
    ...
    validate_every = int(training_cfg.get("validate_every", 1000))
    ...
    val_batch_size = int(training_cfg.get("val_batch_size", micro_batch_size))
    ...
    for epoch in range(epochs):
        ...
        for x, y in loader:
            ...
            if val_loader is not None and train_steps % validate_every == 0:
                logger.info("Evaluating on validation set...")
                val_loss = evaluate_validation_loss(
                    model.module, val_loader, transport, rae, device, autocast_kwargs, world_size
                )
                # Also evaluate EMA model
                val_loss_ema = evaluate_validation_loss(
                    ema, val_loader, transport, rae, device, autocast_kwargs, world_size
                )
                
                logger.info(f"(step={train_steps:07d}) Val Loss: {val_loss:.4f}, Val Loss (EMA): {val_loss_ema:.4f}")
                
                if args.wandb:
                    wandb_utils.log(
                        {
                            "val loss": val_loss,
                            "val loss (ema)": val_loss_ema,
                            "train_val_loss_ratio": avg_loss / val_loss if val_loss > 0 else 0,
                        },
                        step=train_steps,
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--val-data-path", type=str, default=None, help="Path to the validation dataset root.")
